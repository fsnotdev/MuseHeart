...
class ServerManagerView(disnake.ui.View):
    ...
    def build_embed(self, interaction: Union[disnake.MessageInteraction, CustomContext]):
        ...
        embed = disnake.Embed(
            ...
            description=f"```{self.current_guild.name}```\n"
                        ...
                        f"**I joined at:** <t:{joined_at}:f> - <t:{joined_at}:R>\n"
        ...
        if interaction.guild.id == self.current_guild.id and interaction.bot.user.id == self.bot.user.id:
            embed.description += f"\n```ansi\n[32;1mI'm in the current server!```"
    ...
    def rebuild_components(self):
        ...
        leave = disnake.ui.Button(label="Remove", emoji="♻️", style=disnake.ButtonStyle.red)
        leave.callback = self.leave_guild
        self.add_item(leave)

        stop = disnake.ui.Button(label="Stop", emoji="⏹️", style=disnake.ButtonStyle.blurple)
